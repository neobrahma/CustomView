...
class RemoteControlRectView : AbstractRemoteControlView {
    ...
    private val paintXMode = Paint(Paint.ANTI_ALIAS_FLAG)
    ...
    private fun drawHover(canvas: Canvas, position: Int) {
        if (position != -1) {
            ...
            canvas.drawRoundRect(rectShape, roundRect, roundRect, paintXMode)
            //draw destination
            paintXMode.apply {
                style = Paint.Style.FILL
            ...
            canvas.drawRect(rect, paintXMode)
            paint.xfermode = null
        }
    }
